...
<head>
<meta charset="utf-8">
<title>TOON vs JSON Benchmark v8.1</title>
<style>
...
<body>
<h1>TOON vs JSON Benchmark v8.1</h1>
...
Key Updates:
  1. Fixed UI Level Select Glitch: Added explicit select box styling (height, min-width, background)
     and unified JS state updates to prevent browser layout collapse on option injection.
  2. Restored 100 Repeats: Added repeat count 100 to dropdowns and API handlers.
  3. Paired Cache Execution: Runs JSON and TOON dynamically in randomized pairs in 1 pass.
"""
import os
import random
import statistics
...
<style>
  ...
  button { background: #222; color: white; border: none; cursor: pointer; height: 35px; }
  button:disabled { background: #999; }
  table { border-collapse: collapse; width: 100%; margin-top: 16px; font-size: 12.5px; }
  th, td { border: 1px solid #ddd; padding: 5px 8px; text-align: center; }
  th { background: #f5f5f5; }
...
function updateLevels() {
  ...
  if (levels.length > 0) {
    ...
    levelSel.innerHTML = optionsHtml;
    levelSel.value = levels.includes(5) ? '5' : String(levels[0]);
  } else {
    levelSel.innerHTML = '';
  }
}
...
function toggleFields() {
  ...
  document.getElementById('encodingField').style.display = (isCache || isResearch) ? 'none' : 'flex';
  document.getElementById('levelField').style.display = (isCache || isResearch || enc === 'identity') ? 'none' : 'flex';
  document.getElementById('sourceModeField').style.display = isCache ? 'none' : 'flex';
  document.getElementById('modeField').style.display = isCache ? 'flex' : 'none';
